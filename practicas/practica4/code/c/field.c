...
void pasoMC(int L, int **s, double *expVec, int *E, int *m, double B, double T);
...
int main(int argc, char *argv[])
{
    ...
    double Bi = atof(argv[6]);
	double deltaB = atof(argv[7]);
	...
	for(i = 0 ; i < L ; i++)
		for(j = 0 ; j < L ; j++)
			E = E - s[i][j]*s[plus[i]][j] - s[i][j]*s[i][plus[j]] - Bi*s[i][j];
	printf("E = %d\n", E);
	...
	for(B = Bi; B >= 0; B -= deltaB) {
		
		expVec[0] = exp(-4./T);
		expVec[1] = exp(-8./T);
		for(k = 0; k <= N; k++) 
			pasoMC(L, s, expVec, &E, &M, B, T);
		...
		fprintf(stream, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
				B, EMedia/(L*L), mMedia, cp, chi, m2Media, m4Media);	

		printf("%.3lf    %.3lf     %.3lf    %.8lf     %.8lf\n", B, EMedia/(L*L), mMedia, m2Media, m4Media);
